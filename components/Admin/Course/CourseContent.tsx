...
const CourseContent: FC<Props> = ({
    ...
    const addNewSection = () =>{
        if (
            ...
            toast.error("Pleasefill all the fields first")
        } else {
                        ...
                        <>
                            <div className={`w-full bg-[#cdc8c817]p-4 ${showSectionInput ? "mt-10" : "mb-0"}`}>
                                {showSectionInput && (
                                    ...
                                    font-Poppins cursor-pointer dark:text-white text-black bg-tranparent outline-none`}
                                ...
                                <div className='flex w-full items-center justify-between my-0'> {isCollapsed[index] ? (
                                    <>
                                        {item.title ? (
                                ...
                                {!isCollapsed[index] && (
                                    <>
                                        <div className='my-3'>
                                            ...
                                            <input type="text" placeholder="Project Plan.." className={`${styles.input}`}
                                                value={item.title}
                                                onChange={(e) => {
                                                    const updatedData = [...courseContentData];
                                        ...
                                        <div className='my-3'>
                                            ...
                                            <input type="text" placeholder="Video.." className={`${styles.input}`}
                                                value={item.videoUrl}
                                                onChange={(e) => {
                                                    const updatedData = [...courseContentData];
                                        ...
                                        <div className='my-3'>
                                            ...
                                            <textarea rows={8} cols={30} placeholder="write.." className={`${styles.input} !h-min py-2`}
                                            ...
                                            item?.links.map((link: any, linkIndex: number) => (
                                                <div className='mb-3 block'>
                                                    <div className='w-full flex items-center justify-between'>
                                                        ...
                                                        <AiOutlineDelete className={`${linkIndex === 0 ? "cursor-no-drop" : "cursor-pointer"
                                                            } text-black dark:text-white text--[20px]`}
                                                            onClick={() =>
                                                                linkIndex === 0 ? null : handleRemoveLink(index, linkIndex)
                                                            } />
                                                    </div>
                                                    <input type="text" placeholder="Source Code... (Link title)" className={`${styles.input}`}
                                                        value={link.title}
                                                        onChange={(e) => {
                                                            const updatedData = [...courseContentData];
                                                    ...
                                                    <input type="url" placeholder="source code url..(Link Url)" className={`${styles.input} mt-6`}
                                                        value={link.url}
                                                        onChange={(e) => {
                                                            const updatedData = [...courseContentData];
